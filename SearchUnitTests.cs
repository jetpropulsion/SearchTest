...
namespace SearchTest
{
	...
	public class SearchUnitTests
	{
		...
		********************************************************************************************************************
		...
		#region Dimensioning
		//All of these definitions below will be combined, providing different test environments
		...
		public class LargePattern : SearchTestParams.PatternDefinition { public LargePattern() : base(nameof(LargePattern), 2048, 4096) { } };

		//Buffer definitions
		public class SmallBuffer : SearchTestParams.BufferDefinition { public SmallBuffer() : base(nameof(SmallBuffer), 1048576 * 2, 1048576 * 4) { } };
		public class StandardBuffer : SearchTestParams.BufferDefinition { public StandardBuffer() : base(nameof(StandardBuffer), 1048576 * 8, 1048576 * 16) { } };
		public class LargeBuffer : SearchTestParams.BufferDefinition { public LargeBuffer() : base(nameof(LargeBuffer), 1048576 * 16 * 2, 1048576 * 16 * 24) { } };
		...
		public class SmallIterations : SearchTestParams.IterationsDefinition { public SmallIterations() : base(nameof(SmallIterations), 1, 1) { } };
		...
		public void TestAllDerivates()
		{
			...
			//string stateFilePath = @"C:\Work3\SearchTest\bin\x64\Release\net7.0\20230829-024029-3465-Raita.state";
			if (!string.IsNullOrWhiteSpace(stateFilePath) && File.Exists(stateFilePath))
			{
				this.ReadState(stateFilePath);
			}
			...
			//Fetch separately all defined dimensioning types
			...
			int[] definitionCounter = Enumerable.Repeat<int>(0, definitionLengths.Length).ToArray();

			//This piece of code will combine different dimensioning elements
			List<(Type[] types, SearchTestParams.BufferPatternFillDelegate patternFill)> combinations =
				new List<(Type[], SearchTestParams.BufferPatternFillDelegate)>();
			combinations.Add
			((
				new Type[]
				{
					patternDefinitions[definitionCounter[0]],
					bufferDefinitions[definitionCounter[1]],
					distanceDefinitions[definitionCounter[2]],
					iterationDefinitions[definitionCounter[3]]
				},
				delegateDefinitions[definitionCounter[4]]
			));
			int currentIndex = 0;
			int lastIndex = definitionCounter.Length - 1;
			while (true)
			{
				int curr = definitionCounter[currentIndex];
				int next = (++curr) % definitionLengths[currentIndex];
				definitionCounter[currentIndex] = next;
				if (next < curr)
				{
			...
			for (int i = 0; i < combinations.Count; ++i)
			{
				...
				{
							$"{combinations[i].types[0].Name!}",
							$"{combinations[i].types[1].Name!}",
							$"{combinations[i].types[2].Name!}",
							$"{combinations[i].types[3].Name!}",
							$"{combinations[i].patternFill.Method.Name}"
				};
				...
				IterationsDefinition iterations = thisAssembly.CreateInstance(combinations[i].types[3].FullName!) as IterationsDefinition ?? throw new ArgumentNullException();

				SearchTestParams.BufferPatternFillDelegate? patternFillDelegate = combinations[i].patternFill;

				SearchTests.Add(new SearchTestParams( name,	iterations, pattern, buffer, distance, bufferPatternFill: patternFillDelegate ));
			}
			...
			//Pull the types only from statistics dictionary keys
			...
			for (int testIteration = 0; testIteration < SearchTests.Count; ++testIteration)
			{
				...
				WriteStatistics(subTotalTypes, referenceSearchType, subTotal);
			...
			WriteStatistics(totalTypes, referenceSearchType, totalTotal);
		...
		#region Search State Serialization/Deserialization
		public void SaveState
		(
			string path,
			Type type,
			Type referenceSearchType,
			ReadOnlySpan<byte> testPattern,
			ReadOnlySpan<byte> testBuffer, int bufferSize,
			IReadOnlyList<int> generatedOffsets,
			IReadOnlyList<int> referenceOffsets,
		...
		)
		{
			...
			using (BinaryWriter bw = new BinaryWriter(bs, Encoding.UTF8, false))
			{
				...
				//Generated offsets
				stateMessages.Add($"generated offsets: {generatedOffsets.Count}");
				bw.Write(generatedOffsets.Count);
				generatedOffsets.ToList().ForEach(x => bw.Write(x));
				...
				referenceOffsets.ToList().ForEach(x => bw.Write(x));

				//Current ISearch offsets
				stateMessages.Add($"current ({type.FullName}) offsets: {searchOffsets.Count}");
				bw.Write(searchOffsets.Count);
				searchOffsets.ToList().ForEach(x => bw.Write(x));
		...
		public void ReadState(string path)
		{
			...
			using (FileStream fs = File.Open(path, fso))
			using (BufferedStream bs = new BufferedStream(fs))
			using (BinaryReader br = new BinaryReader(bs, Encoding.UTF8, false))
			{
				...
				for (int i = 0; i < restoredGeneratedOffsetsCount; ++i)
				{
					...
					{
						Trace.WriteLine($"{nameof(restoredGeneratedOffsets)}: OFFSET OUT OF BOUNDS at index {i}, offset is {offset}");
					}
					else if (!restoredBuffer.Slice(offset, restoredPatternLength).Span.SequenceEqual(restoredPattern.Span))
					{
						Trace.WriteLine($"{nameof(restoredGeneratedOffsets)}: PATTERN IS ABSENT for offset at index {i}, offset is {offset}");
					}
				...
				for (int i = 0; i < restoredOffsetsCount; ++i)
				{
					...
					if (offset < 0 || offset > restoredBufferSize - restoredPatternLength)
					{
						Trace.WriteLine($"{nameof(restoredOffsets)}: OFFSET OUT OF BOUNDS at index {i}, offset is {offset}");
					}
					else if (!restoredBuffer.Slice(offset, restoredPatternLength).Span.SequenceEqual(restoredPattern.Span))
					{
				...
				for (int i = 0; i < recalculatedOffsets.Count; ++i)
				{
					...
					if (offset < 0 || offset > restoredBufferSize - restoredPatternLength)
					{
						Trace.WriteLine($"{nameof(recalculatedOffsets)}: OFFSET OUT OF BOUNDS at index {i}, offset is {offset}");
					}
					else if (!restoredBuffer.Slice(offset, restoredPatternLength).Span.SequenceEqual(restoredPattern.Span))
					{
		...
		} //END: void ReadState(string path)
		#endregion

		#region Report Statistics
		public void WriteStatistics(IReadOnlyDictionary<Type, SearchStatistics> statistics, Type referenceSearchType, SearchStatistics total)
		{
		...
		} //END: void AggregateStatistics(...)
		#endregion

		//Method will terminate execution if offset collections are different
		public static bool EqualOffsets(string firstName, in IReadOnlyCollection<int> first, string secondName, in IReadOnlyCollection<int> second, bool traceOffsets = false)
		{
			...
			if (traceOffsets)
			{
				foreach (int firstOffset in firstOnly)
				{
					Trace.WriteLine($"{firstName} {firstOffset}");
				}
				foreach (int secondOffset in secondOnly)
				{
					Trace.WriteLine($"{secondName} {secondOffset}");
				}
			}

			return false;

		} //END: static bool EqualOffsets(...)
